...
def depthFirstSearch(problem):
    ...
    while not open.isEmpty():
        x = open.pop()                          # x[0] current node, x[1] = action
        if x[0] not in closed:
            closed.append(x[0])
            if problem.isGoalState(x[0]):       # Goal found!
                return x[1]                     
            else:                               # Goal Not found
                children = problem.getSuccessors(x[0])
                for kid in children:                    # kid[0] = kid state, kid[1] = kid action
                    if kid[0] not in closed:    
                        open.push((kid[0], (x[1] +[kid[1]]))) # When pushed onto stack goes to top/left
    return x[1]
    util.raiseNotDefined()
...
def breadthFirstSearch(problem):
    ...
    while not open.isEmpty():
        x = open.pop()                          # x[0] = current node x[1] = aciton
        if x[0] not in closed:
            closed.append(x[0])
            if problem.isGoalState(x[0]):       # Goal Found!
                return x[1]
                ...
                for kid in children:
                    if kid[0] not in closed:
                        open.push((kid[0], (x[1] +[kid[1]])))   # when pushed in the queue, goes to back/right
    return x[1]
    util.raiseNotDefined()
